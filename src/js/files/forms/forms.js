  ...
  document.body.addEventListener("focusin", function (e) {
    ...
    if (
      ...
      if (targetElement.type === "file") {
        if (parentEl.classList.contains("_form-error")) {
          parentEl.classList.remove("_form-error");
        }
        if (targetElement.classList.contains("_form-error")) {
          targetElement.classList.remove("_form-error");
        }
        if (targetElement.nextElementSibling) {
          if (
            targetElement.nextElementSibling.classList.contains("form__error")
          ) {
    ...
    } else if (parentEl.classList[0] === "select__body") {
      if (parentEl.nextElementSibling) {
        if (parentEl.nextElementSibling.classList.contains("form__error")) {
          parentEl.nextElementSibling.remove();
        }
      }

      let arr = parentEl.parentElement.classList;
      if (arr.contains("_form-error")) {
        arr.remove("_form-error");
      }
    }
  });
...
export let formValidate = {
  getErrors(form) {
    ...
    if (formRequiredItems.length) {
      formRequiredItems.forEach((formRequiredItem, index) => {
        ...
        if (checked && formRequiredItems.length - 1 === index) {
          this.removeError(formRequiredItems[firstInd]);
        } else if (first && formRequiredItems.length - 1 === index) {
          error++;
          this.addError(formRequiredItems[firstInd]);
        }
      });
    }
    return error;
