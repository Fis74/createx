...
class SelectConstructor {
  ...
  selectInit(originalSelect, index) {
    const _this = this;
    // Создаем оболочку
    let selectItem = document.createElement("label");
    ...
    if (this.getSelectPlaceholder(originalSelect)) {
      ...
      if (this.getSelectPlaceholder(originalSelect).label.show) {
        selectItem.insertAdjacentHTML(
          "afterbegin",
          `<span class="${this.getSelectPlaceholder(originalSelect).classModifSpan.text ? this.getSelectPlaceholder(originalSelect).classModifSpan.text : this.selectClasses.classSelectLabel}">${this.getSelectPlaceholder(originalSelect).label.text ? this.getSelectPlaceholder(originalSelect).label.text : this.getSelectPlaceholder(originalSelect).value}</span>`
        );
      }

      if (this.getSelectPlaceholder(originalSelect).spanText.show) {
        selectItem.insertAdjacentHTML(
          "afterbegin",
          `<span class="${this.getSelectPlaceholder(originalSelect).classModifSpan.text ? this.getSelectPlaceholder(originalSelect).classModifSpan.text : this.selectClasses.classSelectLabel}">${this.getSelectPlaceholder(originalSelect).spanText.text ? this.getSelectPlaceholder(originalSelect).spanText.text : this.getSelectPlaceholder(originalSelect).value}</span>`
        );
      }
    }
  ...
  selectBuild(originalSelect) {
    ...
    originalSelect.dataset.classModif
      ...
      : null;
    originalSelect.dataset.classModifLabel
      ? selectItem.classList.add(`${originalSelect.dataset.classModifLabel}`)
      : null;
      ...
      if (targetType === "click") {
        if (!originalSelect.disabled) {
          if (
            ...
            // Обработка клика на label если он есть
            this.selectAction(selectItem);
  ...
  getSelectPlaceholder(originalSelect) {
    ...
    if (selectPlaceholder) {
      return {
        ...
        spanText: {
          show: selectPlaceholder.hasAttribute("data-span-text"),
          text: selectPlaceholder.dataset.spanText,
        },
        classModifSpan: {
          show: selectPlaceholder.hasAttribute("data-class-modif-span"),
          text: selectPlaceholder.dataset.classModifSpan,
        },
      };
    }
  }
