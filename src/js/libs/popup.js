...
flsModules.menu = new Popup({
  logging: true,
  init: true,
  // Для кнопок
  attributeOpenButton: "data-menu-popup", // Атрибут для кнопки, которая вызывает попап
  attributeCloseButton: "data-menu-popup-close", // Атрибут для кнопки, которая закрывает попап
  // Для сторонних объектов
  // Изменение классов
  classes: {
    popup: "menu-popup",
    // popupWrapper: 'popup__wrapper',
    popupContent: "menu-popup__content",
    popupActive: "menu-popup_show", // Добавляется для попапа, когда он открывается
    bodyActive: "menu-popup-show", // Добавляется для боди, когда попап открыт
  },
  focusCatch: true, // Фокус внутри попапа зациклен
  closeEsc: true, // Закрытие по ESC
  bodyLock: true, // Блокировка скролла
